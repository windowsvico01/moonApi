...
module.exports = {
  ...
  getCoupleInfo: async (req) => {
    ...
    const coupleInfo = await baseCouple.getCoupleInfo(couple_key);
    if (coupleInfo.code*1 === 1000) {
      if (coupleInfo.data.length > 0) { // 有couple
        ...
        if (targetUid) {
          targetUserInfo = await baseUser.getUserInfoByUid(targetUid);
        }
  ...
  editCoupleInfo: async (req) => {
    ...
    const updateRes = await baseCouple.updateCoupleInfo(couple_key, params);
