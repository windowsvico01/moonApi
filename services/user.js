...
const { getTokenFromReq, to } = require('../utils/tools');
...
module.exports = {
  ...
  setTargetKey: async (from , to) => {
    ...
    const params = [to];
    return await to(db.queryAsync(updateSql, params));
  },
  ...
  bind: async (req) => {
    ...
    const tUser = await to(baseUser.getUserInfoByKey(key));
    ...
    const fromTargetRes = await to(baseUser.setTargetUid(uid, targetUid, key));
    if (fromTargetRes.code !== 1000) {
      res.data = baseUser.getError(3001);
      return res;
    };
    const toTargetRes = await to(baseUser.setTargetUid(targetUid, uid, key));
    if (toTargetRes.code !== 1000) {
      res.data = baseUser.getError(3001);
      return res;
    };
    const addRes = await to(Couple.insertNewCouple(key, `${targetUid},${uid}`));
    if (addRes.code === 1000) {
      res.status = 200;
      res.data.code = 1000;
  ...
  getUserInfoByWxId: async (req) => {
    ...
    const userInfo = await to(baseUser.getUserInfoByWxId(uid));
    if (userInfo.code === 1000) {
      res.status = 200;
      res.data.code = 1000;
      res.data.data = userInfo.data[0];
  ...
  getUserInfo: async (req) => {
    ...
    const tokenInfo = await to(baseUser.decodeToken(token));
    if (tokenInfo.code === 1002) {
      res.data = tokenInfo;
      return res;
    }
    const data = await to(baseUser.getUserInfoByToken(token));
  ...
  updateUserInfo: async (req) => {
    ...
    const updateRes = await to(baseUser.updateUserInfo(uid, params));
  ...
  signIn: async (req, resolve) => {
    ...
    const userInfo = await to(baseUser.getUserInfoByWxId(wxId));
    if (userInfo.code !== 1000) {
    ...
    const result = await to(baseUser.updateUserToken(userInfo.data[0].uid, token));
    if (result.code !== 1000) {
      res.data.msg = '登录失败';
      return res;
    }
  ...
  signUp: async (req, resolve) => {
    ...
    const result = await to(baseUser.insertUser(addParams));
    if (result.code !== 1000) {
      res.data = {
        'code': -1,
        'msg': '注册失败'
  ...
  getOpenInfo: async (req) => {
    ...
    const openInfo = await to(baseUser.requestOpenInfo(code));
